...
for iter = 2:maxiter
    ...
    model = maximization(data,R);         %EM算法的M step，X表示数据矩阵，R表示类别矩阵，model是结构体，表示模型，其属性是模型的参数
    [R, llh(iter)] = expectation(data,model);            %EM算法的E step，X表示数据矩阵，model表示模型结构体，R表示返回的隶属度矩阵，llh表示似然函数的目标值
...
Q = U'\X;                                           %Q=U'的逆与X的乘积
q = dot(Q,Q,1);                                     %dot表示点乘之后求列和
c = d*log(2*pi)+2*sum(log(diag(U)));                
y = -(c+q)/2;
%% 
function s = logsumexp(X, dim)
% 计算 log(sum(exp(X),dim))
if nargin == 1, 
